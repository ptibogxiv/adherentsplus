 ...
 */
class AdherentPlus extends CommonObject
{
  ...
  var $linkedmembers=array();
   ...
   /**
    ...
    /**
     *    Unlink member to another member
     *
     *    @param     int	$userid        	Id of user to link to
     *    @return    int					1=OK, -1=KO
     */
...
require_once DOL_DOCUMENT_ROOT.'/product/class/product.class.php';
    ...
    $sql.= " WHERE ISNULL(d.fk_parent) ";
		...
		if ($resql)
		{
            ...
            while ($obj = $this->db->fetch_object($resql))
            {
                ...
                $linkedmember->note_private		= $obj->note_private;
                $linkedmember->note_public		= $obj->note_public;

                $linkedmember->typeid			= $obj->fk_adherent_type;
                $linkedmember->type				= $obj->type;
                $linkedmember->need_subscription 	= $obj->subscription;              
     ...
     *		@return	int					<0 if KO, 0 if nothing done, >0 if OK
     */
    public function revalidate($user)
    {
        global $langs,$conf;
