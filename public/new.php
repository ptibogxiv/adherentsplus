...
// Security check
if (empty($conf->adherentsplus->enabled)) accessforbidden('',0,0,1);
...
print load_fiche_titre($langs->trans("NewSubscription").' '.$season);
...
{
    $listoftype=$adht->liste_array_alt();
    ...
    foreach ($adht->liste_array_alt() as $type) {
    if ($conf->global->ADHERENT_SUBSCRIPTION_PRORATA < '2') { 
        $montant=$type[price];
        }
        else {     
        $montant=$tx*$type[price];
        }
    $amount=price($montant+$type[welcome]);
    print '<input type="radio" class="moreaction" id="type" name="type" value="'.$type[rowid].'" ';
    if ($type[automatic]!=1) { print ' disabled ';}
    print'> '.$type[label];
    if ($type[note]!=NULL) {
    print ', '.$type[note];
    }
    print '<input type="hidden" id="'.$type[rowid].'" name="type_amount'.$type[rowid].'" value="'.$amount.'"><br>';
    }
    print '<script type="text/javascript">
   
    jQuery(document).ready(function () {
       var typeid = $( "input[name=type]:checked" ).val();
        $("input[name=type]").change(function(typeid) {
        var typeid = $( "input[name=type]:checked" ).val();
        var test = jQuery("#"+typeid).val();
        jQuery(".amount").val( test );
        });
    });
    </script>';    
...
// Validity
print '<tr><td class="titlefield">'.$langs->trans('DateSubscription').'</td><td>';
print $debut.' '.$langs->trans('to').' '.$fin.'</td></tr>'."\n";
